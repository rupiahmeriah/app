...
export default function SetupPage({ session }: { session: Session }) {
  ...
  async function setupAndRedirectToBrickWidget() {
    ...
    window.location.assign(
      `https://cdn.onebrick.io/sandbox-widget/v1/?accessToken=${publicToken}&region=id&user_id=${user?.id}&redirect_url=${process.env.NEXT_PUBLIC_APP_URL}/api/saveUserAccessToken`
    );
  }
